...
object GenerateTestContentUtils {
    ...
    fun generateArticle(id: Int): ArticleData{
        return ArticleData(id, "测试，id: $id",
            "　伏尔泰曾经说过，坚持意志伟大的事业需要始终不渝的精神。这不禁令我深思。 既然如此， " +
                    "问题的关键究竟为何? 歌德在不经意间这样说过，没有人事先了解自己到底有多大的力量，" +
                    "直到他试过以后才知道。带着这句话，我们还要更加慎重的审视这个问题： 德谟克利特曾经" +
                    "说过，节制使快乐增加并使享受加强。这启发了我， 总结的来说， 了解清楚测试到底是一" +
    ...
    fun generateCollectionIdList(): ArrayList<NetWorkResponse.Collection>{
        return arrayListOf(NetWorkResponse.Collection(1), NetWorkResponse.Collection(2))
    }
    ...
    fun generateCommentDataList(): ArrayList<CommentData>{
        return arrayListOf(
    ...
    fun generateLifeIndexData(): LifeIndexData{
        return LifeIndexData("好（测试）", "好（测试）", "好（测试）", "好（测试）", "好（测试）")
    }
